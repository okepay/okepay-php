...
class Communicator
{
    ...
    const API_URL_BASE_DOMAIN = 'okepay.info';
    const DEFAULT_COMMUNICATION_HANDLER = '\OkePay\CommunicationAdapter\CurlCommunication';
    ...
    /**
     * @var string The OkePay instance name.
     */
    protected $instance;
    /**
    ...
    public function __construct($instance, $apiSecret, $communicationHandler, $apiBaseDomain)
    {
        ...
        if (!class_exists($communicationHandler)) {
            throw new OkePayException('Communication handler class ' . $communicationHandler . ' not found');
        }
     ...
     * @param \OkePay\Models\Base $model  The model which has the same functionality like a filter.
     *
     * @return \OkePay\Models\Base[]|\OkePay\Models\Base An array of models or just one model which
     *                                                                       is the result of the API call
     * @throws \OkePay\OkePayException An error occurred during the OkePay Request
     */
    public function performApiRequest($method, \OkePay\Models\Base $model)
    {
        ...
        if (!isset($response['body']['data']) || !is_array($response['body']['data'])) {
            if (!isset($response['body']['message'])) {
                throw new \OkePay\OkePayException('OkePay PHP: Configuration is wrong! Check instance name and API secret', $response['info']['http_code']);
            }
            throw new \OkePay\OkePayException($response['body']['message'], $response['info']['http_code']);
        }
     ...
     * @throws \OkePay\OkePayException The method is not implemented yet.
     */
